...
foodshop %>%
  ...
  select(name, type, open_date, address,open_year,close_year,city)
...
#3.업종별 개업 비율
foodshop %>%
  filter(!is.na(open_date)&!is.na(type)&!is.na(city)) %>% #결측치제외
  group_by(type) %>%
...
#7.개업이 많았던 연도
foodshop %>%
  filter(!is.na(open_date)&!is.na(city))%>% #결측치제외
  group_by(open_year) %>%
  summarise(n=n()) %>% #범주빈도계산
  arrange(desc(n)) %>%
  head(5)
...
#8.폐업이 많았던 연도
foodshop %>%
  filter(!is.na(close_date)&!is.na(city))%>% #결측치제외
  group_by(close_year) %>%
  summarise(n=n()) %>% #범주빈도계산
  arrange(desc(n)) %>%
  head(5)
...
city_business<-foodshop %>%
  filter(!is.na(open_date)&!is.na(city)&status=="영업") %>% #결측치제거
  group_by(city) %>%
  summarise(n=n())
...
#14,32개 시의 음식점수 막대그래프
ggplot(data = city_business, aes(x=reorder(city,n),y=n))+
  geom_col()+
  coord_flip()+#막대 90도회전
  xlab("영업시")+
  ylab("영업 음식점 수")
...
foodshop %>%
  filter(!is.na(open_date)&!is.na(city)) %>% #결측치제거
  filter(type%in%c("기타","경양식","분식","일식","중국식","호프/통닭"))%>%
  filter(status=="영업") %>% #영업만 추출
  group_by(type,city) %>%
  summarise(n=n()) %>%
  mutate(total=sum(n),pct=round(n/total*100,1))%>%#범주별비율계산
  group_by(type) %>%filter(pct==max(pct))%>%arrange(desc(n))#type별 city비율이 가장 높은 데이터 추출
