...
foodshop <- read.csv("seoul_food_2025.csv", na="", stringsAsFactors = F)
...
foodshop$district <- ifelse(foodshop$district%in%c("도 제","시 고","시 단", "시 망","시 분","시 수","시 영","시 원","시 일"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인
